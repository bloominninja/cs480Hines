...
//glm::mat4 cube;
//Camera Controlls
glm::vec3 Position= glm::vec3 (20, 40.0, 0);
...
bool setsquare1=false;   
bool setsquare2=false; 
float look=1;
queue <glm::vec3> jumpto;

// Points and scoring variables.
    // Score is for goals made
    // Points are for performance: bank shots, deflections, etc.
int player1Score = 0;
int player2Score = 0;
int player1Points = 5000;
int player2Points = 10000;

    // Persistent top 10 leaderboard.
        // Investigate collapsing these into one array once scoring system is complete.
//char top10Initials[][4];
//char top10Scores[][6];
...
int puckColl = COL_PADDLE|COL_WALL|COL_BOX;
int boundColl= COL_PADDLE;
int boxColl=COL_PUCK;
...
void updatePaddleKeyboardMove(bool ai);
...
void createBoard(float x, float y, float z, float gap1, float gap2);
void updateItem(model_pack& pack);
btVector3 GetOGLPos(int x, int y);
bool pythagdis(float x,float y, float z);
...
//Set wait time for start
int total_ticks=185;
int clock_tick=total_ticks;
//--Main
int main(int argc, char **argv)
   ...
   //Create Gameboard
   createBoard(board_x,board_y,board_z,board_width,board_width); 
   
   //Create Puck
   puck.dim=btVector3(1,.2,1);
   puck.shape= new btCylinderShape(puck.dim);
   applyPhysics(puck.shape,puck.body,0,20,0,5);
   ...
   applyPhysics(paddle1.shape,paddle1.body,0,1.1,board_x-(.25*board_x),500);
	dynamicsWorld->addRigidBody(paddle1.body, COL_PADDLE, paddleColl);
	paddle1.body->setRestitution(.2);

	spaddle1.dim=btVector3(1,1,1);		
	spaddle1.shape= new btBoxShape(paddle1.dim);
   applyPhysics(spaddle1.shape,spaddle1.body,0,500,0,500);
	dynamicsWorld->addRigidBody(spaddle1.body, COL_PADDLE, paddleColl);
	spaddle1.body->setRestitution(.2);

	//Create Paddle 2
	paddle2.dim=btVector3(1,1,1);		
	paddle2.shape= new btCylinderShape(paddle2.dim);
   applyPhysics(paddle2.shape,paddle2.body,0,1.1,(.25*board_x)-board_x,500);
	dynamicsWorld->addRigidBody(paddle2.body, COL_PADDLE, paddleColl);
	paddle2.body->setRestitution(.2);
	
	spaddle2.dim=btVector3(1,1,1);		
	spaddle2.shape= new btBoxShape(paddle1.dim);
   applyPhysics(spaddle2.shape,spaddle2.body,0,5000,0,500);
	dynamicsWorld->addRigidBody(spaddle2.body, COL_PADDLE, paddleColl);
	spaddle2.body->setRestitution(.2);
	
	
	//create Bound Walls to prevent vibro bouncing
   		paddleWalls[0].dim=btVector3(board_z,.5,.1);
  	 		paddleWalls[0].shape= new btBoxShape (paddleWalls[0].dim);        
			applyPhysics(paddleWalls[0].shape,paddleWalls[0].body,0, 0, 0,0);
			dynamicsWorld->addRigidBody(paddleWalls[0].body, COL_BOUND, boundColl);
			paddleWalls[0].body->setRestitution(0);
			
			paddleWalls[1].dim=btVector3(board_z,.3,.07);
  	 		paddleWalls[1].shape= new btBoxShape (paddleWalls[1].dim);        
			applyPhysics(paddleWalls[1].shape,paddleWalls[1].body,0, 0, board_x-1,0);
			dynamicsWorld->addRigidBody(paddleWalls[1].body, COL_BOUND, boundColl);
			paddleWalls[1].body->setRestitution(0);
			
			
			paddleWalls[2].dim=btVector3(board_z,.3,.07);
  	 		paddleWalls[2].shape= new btBoxShape (paddleWalls[2].dim);        
			applyPhysics(paddleWalls[2].shape,paddleWalls[2].body,0, 0, -(board_x-1),0);
			dynamicsWorld->addRigidBody(paddleWalls[2].body, COL_BOUND, boundColl);
			paddleWalls[2].body->setRestitution(0);
			
			
						paddleWalls[3].dim=btVector3(.07,.3,board_x);
  	 		paddleWalls[3].shape= new btBoxShape (paddleWalls[3].dim);        
			applyPhysics(paddleWalls[3].shape,paddleWalls[3].body,board_z-1, 0,0,0);
			dynamicsWorld->addRigidBody(paddleWalls[3].body, COL_BOUND, boundColl);
			paddleWalls[3].body->setRestitution(0);
			
						paddleWalls[4].dim=btVector3(.07,.3,board_x);
  	 		paddleWalls[4].shape= new btBoxShape (paddleWalls[4].dim);        
			applyPhysics(paddleWalls[4].shape,paddleWalls[4].body,-(board_z-1), 0,0,0);
			dynamicsWorld->addRigidBody(paddleWalls[4].body, COL_BOUND, boundColl);
			paddleWalls[4].body->setRestitution(0);
/*
			paddleWalls[0].dim=btVector3(board_z,.3,.1);
  	 		paddleWalls[0].shape= new btBoxShape (paddleWalls[0].dim);        
			applyPhysics(paddleWalls[0].shape,paddleWalls[0].body,0, 0, 0,0);
			dynamicsWorld->addRigidBody(paddleWalls[0].body, COL_BOUND, boundColl);
			paddleWalls[0].body->setRestitution(0);*/
...
void render()
    ...
    addModel(2,ind,9,paddleWalls[0].model);

    if(setsquare1)
    {
    addModel(2,ind,3,paddle1.model);
    }
    else if(!setsquare1)
    {
    ...
    if(setsquare2)
    {
     addModel(2,ind,3,paddle2.model);
    }
    else if(!setsquare2)
    {
    ...
    //addModel(2,ind,3,spaddle1.model);
    //addModel(2,ind,3,spaddle2.model);
    //clean up
...
void update()
    ...
    if(!stop_paddles)
    {
		 ...
		 updatePaddleMouseMove();
	 	 ...
	 	 updatePaddleKeyboardMove(tog_ai);
 	 }
    ...
    puck.body->getMotionState()->getWorldTransform(trans);
	 ...
	 else if(clock_tick==1)
	 {
	  stop_paddles=false;
	  //puck.body->setLinearFactor(btVector3(1,1,1));
	  puck.body->setAngularFactor(btVector3(0,0,0));
	 transform = puck.body -> getCenterOfMassTransform();
	 ...
	 {
	 transform = puck.body -> getCenterOfMassTransform();
	 transform.setOrigin(btVector3( transform.getOrigin().getX(),.6, transform.getOrigin().getZ() ));
	 puck.body -> setCenterOfMassTransform(transform);
	 
	 puck.body->setAngularFactor(btVector3(0,1,0));
    puck.body->setLinearFactor(btVector3(1,0,1));
	 ...
	 //Player 2 Point stuff
	  transform = puck.body -> getCenterOfMassTransform();
	  transform.setOrigin(btVector3(0,1,board_z-(board_z*.5)));
	 puck.body -> setCenterOfMassTransform(transform);
//reset
	 puck.body->setLinearVelocity(btVector3(.0001, 0,.0001));
	 	 player2Score+=1;
	 ...
	 //Player 1 Point stuff
	  ...
	  transform.setOrigin(btVector3(0,1,(board_z*.5)-board_z));
	 puck.body -> setCenterOfMassTransform(transform);
	 //reset
	 puck.body->setLinearVelocity(btVector3(.0001, 0,.0001));	 
	 player1Score+=1;	 	 
	...
	dif=abs(player1Score-player2Score);



	 		gap1=board_width;
	 		gap2=board_width;
			wall[0].dim=btVector3(1,board_y,board_x);
        	wall[1].dim=btVector3(1,board_y,board_x);
        	wall[2].dim=btVector3((board_z/2)-gap1,board_y,1);
        	wall[3].dim=btVector3((board_z/2)-gap1,board_y,1);
        	wall[4].dim=btVector3(gap1*2,board_y,1);
        	wall[5].dim=btVector3((board_z/2)-gap2,board_y,1);
        	wall[6].dim=btVector3((board_z/2)-gap2,board_y,1);
        	wall[7].dim=btVector3(gap2*2,board_y,1);

	
			for(int i=0;i<7;i++)
		{
			natural[i]=wall[i].dim;
			}
		 	...
		 	//Set everything to start point and velocity to 0 
	...
	if(dif==0)
	 {
		...
		for(int i=0;i<7;i++)
		{
        	wall[i].shape->setLocalScaling(wall[i].dim/natural[i]);
	 ...
	 {
	 		gap1=board_width+.05*dif;
	 		gap2=board_width-.05*dif;
			wall[0].dim=btVector3(1,board_y,board_x);
        	wall[1].dim=btVector3(1,board_y,board_x);
        	wall[2].dim=btVector3((board_z/2)-gap1,board_y,1);
        	wall[3].dim=btVector3((board_z/2)-gap1,board_y,1);
        	...
        	for(int i=0;i<7;i++)
			{ 	
			print=natural[i]/wall[i].dim;
		
        	wall[i].shape->setLocalScaling(wall[i].dim/natural[i]);
        	}
        	
        	        	//wall[4].shape->setLocalScaling(wall[4].dim);
        	        	//dynamicsWorld->updateSingleAabb 	(wall[4].body); 
        	//wall[7].shape->setLocalScaling(wall[7].dim);
        	//dynamicsWorld->updateSingleAabb 	(wall[7].body);
	 }
	 else if(player1Score<player2Score)
	 {
	 		gap1=board_width-.05*dif;
        ...
        //	dynamicsWorld->updateSingleAabb 	(wall[7].body);
	 		 ...
	 		 transform = wall[2].body -> getCenterOfMassTransform();
	 		 transform.setOrigin(btVector3((board_z/2)+gap1, -1,board_x));
	 		 wall[2].body -> setCenterOfMassTransform(transform);
			 dynamicsWorld->updateSingleAabb 	(wall[2].body);
			 
	 		 transform = wall[3].body -> getCenterOfMassTransform();
	 		 transform.setOrigin(btVector3(-(board_z/2)-gap1, -1,board_x));
	 		 wall[3].body -> setCenterOfMassTransform(transform); 
	 		 wall[3].shape=new btCylinderShape(paddle1.dim);
			dynamicsWorld->updateSingleAabb 	(wall[3].body);
			
	 		 transform = wall[5].body -> getCenterOfMassTransform();
	 		 transform.setOrigin(btVector3((board_z/2)+gap2, -1, -board_x));
	 		 wall[5].body -> setCenterOfMassTransform(transform);
dynamicsWorld->updateSingleAabb 	(wall[5].body);
	 		 transform = wall[6].body -> getCenterOfMassTransform();
	 		 transform.setOrigin(btVector3((-board_z/2)-gap2, -1, -board_x));
	 		 wall[6].body -> setCenterOfMassTransform(transform);	 
	dynamicsWorld->updateSingleAabb 	(wall[6].body);
	 ...
	 updateItem(paddleWalls[0]);
	 updateItem(paddleWalls[1]);
	 	 updateItem(paddleWalls[2]);
	 	 	updateItem(paddleWalls[3]);
	 	 	updateItem(paddleWalls[4]);
    ...
    updateItem(wall[6]); 
    updateItem(wall[7]); 
   wall[2].model[3][0]=(board_z/2)+gap1; 
	wall[2].model[3][1]=-1;
	wall[2].model[3][2]=board_x;
	wall[3].model[3][0]=-(board_z/2)-gap1; 
	wall[3].model[3][1]=-1;
	wall[3].model[3][2]=board_x;
	
	wall[5].model[3][0]=(board_z/2)+gap2; 
	wall[5].model[3][1]=-1;
	wall[5].model[3][2]=-board_x;
	wall[6].model[3][0]=(-board_z/2)-gap2; 
	wall[6].model[3][1]=-1;
	wall[6].model[3][2]=-board_x;
    // Update the state of the scene

   //Move secondary box 
 	if(setsquare1)
	{
	transform = paddle1.body -> getCenterOfMassTransform();
	spaddle1.body -> setCenterOfMassTransform(transform);
	spaddle1.body->setAngularFactor(btVector3(0,1,0));

    spaddle1.body->setLinearFactor(btVector3(1,0,1));
    updateItem(spaddle1);
	}
	else if(!setsquare1)
	{
	 transform = spaddle1.body -> getCenterOfMassTransform();
	 transform.setOrigin(btVector3(0,900,0));
	 spaddle1.body -> setCenterOfMassTransform(transform);
	 updateItem(spaddle1);
	}   
	...
	if(setsquare2)
	{
	transform = paddle2.body -> getCenterOfMassTransform();
	spaddle2.body -> setCenterOfMassTransform(transform);
	spaddle2.body->setAngularFactor(btVector3(0,1,0));

    spaddle2.body->setLinearFactor(btVector3(1,0,1));
    updateItem(spaddle1);
	}
	else if(!setsquare2)
	{
	 transform = spaddle2.body -> getCenterOfMassTransform();
...
void keyboard(unsigned char key, int x_pos, int y_pos)
  ...
  if(key == 122) //Press z to toggle ai
          {

				tog_ai=!tog_ai;
			}
  if(key == 49) //toggle square puck 1
         {
				setsquare1=!setsquare1;
			}
  if(key == 50) //toggle square puck 2
         {
				setsquare2=!setsquare2;
			}
...
void updatePaddleKeyboardMove(bool ai)
{
	//Set the paddle motions to active and set velocity to 0 *degrade velocity later
		btTransform trans,form;
		paddle1.body->activate(true);
		paddle1.body->setLinearVelocity(btVector3(0, paddle1.body->getLinearVelocity().getY(),0));
		float horizontal=0;
		float vertical=0;
		...
		if(ai)
		{
			//Get puck locations
			puck.body->getMotionState()->getWorldTransform(trans);
			paddle1.body->getMotionState()->getWorldTransform(form);
			//If paddle is close to puck, hit it
			if(pythagdis(abs(trans.getOrigin().getX()-form.getOrigin().getX()),abs(trans.getOrigin().getZ()-form.getOrigin().getZ()),3)&&trans.getOrigin().getZ()<form.getOrigin().getZ())
			{
				vertical-=6;
			}
			//Move up if puck is above/down if below
			if(trans.getOrigin().getX()>form.getOrigin().getX()&&trans.getOrigin().getZ()<board_x-2.1)
			{

			 horizontal=+2;
			}
			else if(trans.getOrigin().getX()<form.getOrigin().getX()&&trans.getOrigin().getZ()<board_x-2.1)
			{

			horizontal=-2;
			}

			if(trans.getOrigin().getZ()>board_z/3)    
			{
			vertical-=1;
			}
			else
			...
			if(abs(trans.getOrigin().getZ()+1)>abs(form.getOrigin().getZ()))
			{
			vertical+=4;
			horizontal=0;
			}
	...
	//Apply Velocity	
	paddle1.body->setLinearVelocity(btVector3(horizontal, paddle1.body->getLinearVelocity().getY(),vertical));
...
void updatePaddleMouseMove()
{
	...
	//Call mouse position to get points
 	...
 	//Find where it is
	paddle2.body->getMotionState()->getWorldTransform(trans);
 	//Create a velocity vector between mouse and current mouse position
 	if(look<0)
 	{
	inter=btVector3((-toY-trans.getOrigin().getX())*2,0,(-toX-trans.getOrigin().getZ())*2);
	}
   ...
   btVector3 velocity =paddle2.body->getLinearVelocity();
   btScalar speed = velocity.length();
   btScalar inSpd = inter.length();
	...
	if(speed < 1)
		{
		 paddle2.body->setLinearVelocity((velocity+inter)/2);
		}
   if(speed > 1) 
    	{
       velocity *= 1/speed;
       paddle2.body->setLinearVelocity(inter);          
      }
   ...
   paddle2.body->getMotionState()->getWorldTransform(trans);
	...
	if(trans.getOrigin().getZ()>-1.1)
	{
	btTransform transform = paddle2.body -> getCenterOfMassTransform();
	transform.setOrigin(btVector3(trans.getOrigin().getX(),trans.getOrigin().getY(),-1.1));
	 paddle2.body -> setCenterOfMassTransform(transform);
	}
/*
 ...
 //       stop=true;
        finished=false;
            ...
            while (!jumpto.empty())
		     ...
		     jumpto.push(Position=glm::vec3 (0.0, 40, 20));
		     //jumpto.push(Position=glm::vec3 (0.0, 40, 1));
       ...
       else if(look<0)
       	{
       	  ...
       	  while (!jumpto.empty())
     			...
     			jumpto.pop();
  				...
  				jumpto.push(Position=glm::vec3 (20, 40, 0));
  				look=1;
...
void options()
	...
	//Create the submenu for the menu
		...
		glutAddMenuEntry("Shifted Rectangle", 5);
		glutAddMenuEntry("No Win Scenario", 6);
	...
	//Create menu
		...
		glutAddSubMenu("New Board", submenu_id);  
		glutAddMenuEntry("Pause", 11); 
...
flr.dim=btVector3(board_z,1,board_x);
wall[0].dim=btVector3(1,board_y,board_x);
wall[1].dim=btVector3(1,board_y,board_x);
...
void createBoard(float x, float y, float z, float gap1, float gap2)
{
         ...
         flr.shape= new btBoxShape (flr.dim);        
        	...
        	//flr.body->setCollisionShape(flr.body->getCollisionShape());
       	...
       	wall[0].dim=btVector3(1,y,x);
        	...
        	wall[2].dim=btVector3((z/2)-gap1,y,1);
        	wall[2].shape= new btBoxShape (wall[2].dim);
        	applyPhysics(wall[2].shape,wall[2].body,(z/2)+gap1, -1,x,0);
        	dynamicsWorld->addRigidBody(wall[2].body, COL_WALL, wallColl);
   
        	wall[3].dim=btVector3((z/2)-gap1,y,1);
        	wall[3].shape= new btBoxShape (wall[3].dim);
        	applyPhysics(wall[3].shape,wall[3].body,(-z/2)-gap1, -1, x,0);
        	dynamicsWorld->addRigidBody(wall[3].body, COL_WALL, wallColl);
        	
        	wall[4].dim=btVector3(gap1*2,y,1);
        	...
        	wall[5].dim=btVector3((z/2)-gap2,y,1);
        	wall[5].shape= new btBoxShape (wall[5].dim);
        	applyPhysics(wall[5].shape,wall[5].body,(z/2)+gap2, -1, -x,0);
        	dynamicsWorld->addRigidBody(wall[5].body, COL_WALL, wallColl);

        	wall[6].dim=btVector3((z/2)-gap2,y,1);
        	wall[6].shape= new btBoxShape (wall[6].dim);
        	applyPhysics(wall[6].shape,wall[6].body,(-z/2)-gap2, -1, -x,0);
        	dynamicsWorld->addRigidBody(wall[6].body, COL_WALL, wallColl);
        	
        	
        	wall[7].dim=btVector3(gap2*2,y,1);
...
btScalar m[16]; 
		...
		//pack.shape->setLocalScaling(pack.dim);
...
void ResetGame()
	{
		...
		btTransform transform; 	 
		 	...
		 	//Set everything to start point and velocity to 0 
	 		 ...
	 		 transform = paddle2.body -> getCenterOfMassTransform();
	 		 transform.setOrigin(btVector3(0,1,(.25*board_x)-board_x));
	 		 paddle2.body -> setCenterOfMassTransform(transform);
	 		 paddle2.body->setLinearVelocity(btVector3(.0001, 0,.0001));
	 		 
	 		 transform = paddle1.body -> getCenterOfMassTransform();
	 		 transform.setOrigin(btVector3(0,1,board_x-(.25*board_x)));
	 		 paddle1.body -> setCenterOfMassTransform(transform);
	 		 paddle1.body->setLinearVelocity(btVector3(.0001, 0,.0001));
...
// This function uses the basic GLUT string functions to put up a rudimentary scoreboard.  Hopefully
// this will be replaced later with a version that does proper font rendering with FreeType.
void renderScoreboard()
    {
    // Variable declarations
    int pen_x = 800, pen_y = 900;
...
*/        
    ...
    // Placeholder text rendering code.
        // Build the string to be printed
    string scoreText = "Player 1: ";
    scoreText.append(to_string(player1Score));
    scoreText.append(" | Player 2: ");
    scoreText.append(to_string(player2Score));
    
        // Build a second string to print the player's performance points
    string pointsText = "Player 1: ";
    pointsText.append(to_string(player1Points));
    pointsText.append(" | Player 2: ");
    pointsText.append(to_string(player2Points));
    
        // Temporarily turn off textures and lighting as some sources suggest this can
        // interfere with the proper rendering of text.  Once disabled, print the string.
    glDisable(GL_LIGHTING);
    glDisable(GL_TEXTURE_2D);
    
        // Set text color
    glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
        // Set coordinate on the window for the game score, then print the string.
    glWindowPos2i(glutGet(GLUT_WINDOW_WIDTH)/2 - 120, glutGet(GLUT_WINDOW_HEIGHT) - 75);
    glutBitmapString(GLUT_BITMAP_TIMES_ROMAN_24, reinterpret_cast<const unsigned char*>(scoreText.c_str()));
        // Set coordinates for the player points display, then print the string.
    glWindowPos2i(glutGet(GLUT_WINDOW_WIDTH)/2 - 120, glutGet(GLUT_WINDOW_HEIGHT) - 100);
    glutBitmapString(GLUT_BITMAP_TIMES_ROMAN_24, reinterpret_cast<const unsigned char*>(pointsText.c_str()));
