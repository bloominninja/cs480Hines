...
//    const unsigned char* tmp = reinterpret_cast<const unsigned char*>("Text");
    glDisable(GL_LIGHTING);
    glDisable(GL_TEXTURE_2D);
    glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
    glWindowPos2i(glutGet(GLUT_WINDOW_WIDTH)/2 - 25, glutGet(GLUT_WINDOW_HEIGHT)-75);
...
//reset
	...
	if(dif==0)
	 ...
	 else if(player1Score<player2Score)
	 {
			...
			//cout<< print.getX()<<' '<<print.getY()<<' '<<print.getZ()<<endl;
        	wall[i].shape->setLocalScaling(wall[i].dim/natural[i]);
        	}
...
void updatePaddleKeyboardMove(bool ai)
{
	//Set the paddle motions to active and set velocity to 0 *degrade velocity later
		...
		if(ai)
		{
			...
			if(trans.getOrigin().getZ()>form.getOrigin().getZ())
			{
			vertical+=4;
			horizontal=0;
			}
...
void updatePaddleMouseMove()
{
   ...
   btScalar inSpd = inter.length();
	...
	if(speed < 1)
		{
		 puck.body->setLinearVelocity((velocity+inter)/2);
		}
   if(speed > 1) 
    	{
       velocity *= 1/speed;
       puck.body->setLinearVelocity(inter);          
      }
