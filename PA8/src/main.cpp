...
model_pack physicsSphere;
model_pack physicsCylinder;
model_pack flr;
model_pack wall[4];
...
int main(int argc, char **argv)
    ...
    physicsCylinder.shape= new btCylinderShape(physicsCylinder.dim);
    applyPhysics(physicsCylinder.shape,physicsCylinder.body,0,60,0,1);
...
*/    
    ...
    addModel(1,ind,11,physicsSphere.model);
    addModel(0,ind,8,physicsCylinder.model);
...
//    SUN=glm::scale( glm::mat4(1.0f), glm::vec3(SunScale,SunScale,SunScale));
    ...
    updateItem(physicsSphere);
    updateItem(physicsCube);
    updateItem(physicsCylinder);
    updateItem(flr);
...
void special(int key, int x, int y)
{
    ...
    if(key == GLUT_KEY_LEFT)//A to rotate
        {
        physicsSphere.body->activate(true);
        physicsSphere.body->applyCentralImpulse(btVector3(1, 0, 0));
        physicsSphere.body->applyCentralForce(btVector3(10, 0, 0));
        }
    ...
    if(key == GLUT_KEY_UP)//A to rotate
        {
        physicsSphere.body->activate(true);
        physicsSphere.body->applyCentralImpulse(btVector3(0, 0, 1));
        physicsSphere.body->applyCentralForce(btVector3(0, 0, 10));
        }
    if(key == GLUT_KEY_DOWN)//A to rotate
        {
        physicsSphere.body->activate(true);
        physicsSphere.body->applyCentralImpulse(btVector3(0, 0, -1));
        physicsSphere.body->applyCentralForce(btVector3(0, 0, -10));
